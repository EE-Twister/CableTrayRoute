...
export function authFailureMessage(err, fallback) {
  if (err instanceof SupabaseRequestError && err.status === 429) {
    ...
    if (Number.isFinite(retry) && retry > 0) {
      return `Signup requests are temporarily rate limited. Wait ${retry} seconds, then try again.`;
    }
    return 'Signup requests are temporarily rate limited. Wait about a minute, then try again.';
  }
