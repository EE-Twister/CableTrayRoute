...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const columns = [
    {key:'panel_id',label:'Panel ID',type:'text',group:'Identification',tooltip:'Panel identifier'},
    {key:'circuit_number',label:'Circuit #',type:'number',group:'Identification',tooltip:'Breaker circuit number'},
    {key:'poles',label:'Poles',type:'number',group:'Breaker',tooltip:'Number of poles'},
    {key:'breaker_rating',label:'Breaker Rating (A)',type:'number',group:'Breaker',tooltip:'Breaker ampere rating'},
    {key:'load_description',label:'Load Description',type:'text',group:'Load',tooltip:'Connected load description'},
    {key:'load_type',label:'Load Type',type:'select',options:loadTypes,group:'Load',tooltip:'NEC load category'},
    {key:'connected_va',label:'Connected Load (VA)',type:'number',group:'Load',tooltip:'Connected load in volt-amperes'},
    {key:'demand_factor',label:'Demand Factor',type:'number',group:'Load',tooltip:'Applied demand factor'},
    {key:'demand_va',label:'Demand Load (VA)',type:'number',group:'Load',tooltip:'Demand load in volt-amperes'},
    {key:'notes',label:'Notes',type:'text',group:'Notes',tooltip:'Additional notes'}
  ];
  ...
  const table = TableUtils.createTable({
    ...
    columns,
    onChange:() => { markUnsaved(); updateTotals(); },
    onSave:() => {
      markSaved();
  ...
  table.tbody.addEventListener('input', e => {
    ...
    tr.querySelector('input[name="demand_va"]').value = row.demand_va.toFixed(2);
    updateTotals();
  });
});
