...
function initWorkflowStepNav(){
  ...
  label.textContent=`Step ${idx+1} of ${WORKFLOW_STEPS.length} · ${step.label.replace(/^\d+\.\s*/, '')}`;

  const status=document.createElement('span');
  status.className='workflow-step-nav-status';
  status.setAttribute('role','status');

  const statusDot=document.createElement('span');
  statusDot.className='workflow-step-nav-status__dot';
  statusDot.setAttribute('aria-hidden','true');

  const statusText=document.createElement('span');
  statusText.className='workflow-step-nav-status__text';
  status.append(statusDot,statusText);

  function syncStatus(){
    const context=getWorkflowPageModeContext(page);
    if(!context) return;
    const tone=context.project.tone||'neutral';
    status.className=`workflow-step-nav-status is-${tone}`;
    statusText.textContent=context.project.status;
    status.setAttribute('aria-label',`Project status: ${context.project.status}`);
    status.title=context.project.detail;
  }
