...
function loadArrangements() {
  const stored = getItem(ARRANGEMENTS_KEY, null);
...
function assignedEquipmentGroups() {
  const groups = new Map();
  getEquipment().forEach((item, index) => {
...
function autoLayoutEquipment() {
  ...
  const sourceEquipment = seededFromList
    ? (assignedToActive.length ? assignedToActive : getEquipment().map((item, index) => equipmentFromCatalogItem(item, index)))
...
function showEquipmentDetailModal(eq) {
  const listItem = eq.listTag
    ? getEquipment().find(item => item.tag === eq.listTag) || null
    : null;
