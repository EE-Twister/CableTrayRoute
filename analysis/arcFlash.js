...
export function runArcFlash() {
  ...
  comps.forEach(comp => {
    ...
    const d = Number(comp.enclosure_depth || comp.box_depth) || 508;
    const sizeFactor = Math.cbrt((h * w * d) / (508 * 508 * 508)) || 1;
    const time = clearingTime(comp, Ibf);
    let energy = Cf * sizeFactor * Math.pow(Ibf, 1.2) * time * (gap / 25) * Math.pow(610 / dist, 2);
