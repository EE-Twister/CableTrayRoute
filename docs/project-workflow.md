...
Optimal Route writes a normalized route-result contract to the project store as `latestRouteResults`. DRC, Home, Pull Cards, Spool Sheets, the Workflow Dashboard, guided workflow, and the Report Package Builder read that same project-level route output, so the user can move from routing through validation and field deliverables without re-importing files. Legacy and sample route rows are normalized on read so cable names and raceway assignments are not lost.
