...
- **One-Line** defaults new palette devices to a vertical source-to-load orientation, places clicked components in a readable top-to-bottom stack, supports click-to-connect from device bodies or visible ports, creates provisional cable metadata so drawing is not blocked by cable details, and keeps Properties, Validation, and History in the right-side inspector. The page can now Auto-Build missing one-line components from Equipment and Load List records, Auto Arrange devices source-to-load, Auto Space Equipment rows to standard horizontal branch spacing, show readiness scoring and generated/assumption/link badges, render compact or expanded drawing datablocks with collision-aware placement, color devices by data state, validation, study, or arc-flash result, use a search-first palette with frequent components and category filters, auto-fit after sample, Auto-Build, and cross-probe navigation, apply operating-state open/closed overrides to energized tracing, edit selected connection cable details inline, offer validation quick fixes for common link/cable/voltage issues, and provide direct handoff links to equipment, loads, cable schedule, raceways, routing, and TCC where applicable.
