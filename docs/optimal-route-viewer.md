...
Built-in samples remain isolated in the Optimal Route working session: loading or calculating a sample does not replace the active project's cable or raceway schedules. Sample route results and modeled risers are restored when the page reloads in the same tab.
...
Automatic ductbank routing separates corridor selection from internal conduit assignment. Parallel conduits with the same ductbank, direction, and longitudinal limits are treated as sibling choices even when their drawing centerlines are offset by scheduled row and column. After the route selects a ductbank corridor, the router keeps only one eligible sibling conduit in that cable's graph. It minimizes projected utilization after adding the cable, rejects cable-class or capacity mismatches, and uses row, column, then conduit ID only as stable tie-breakers. This prevents a route from hopping between conduits and distributes equivalent cables across available conduits instead of depending on graph insertion order. Explicit cable raceway assignments and manual paths continue to take precedence.
...
Both the source Raceway Utilization table and Updated Raceway Utilization include cable trays, standalone conduits, and conduits inside ductbanks. Their **Review** actions open the matching workflow: Cable Tray Fill for trays, Conduit Fill for standalone conduits, or Ductbank for an underground conduit. When capacity exceeds the 80% review threshold, the **overloads** summary is an action: select it to open the updated-utilization section, filter the table to affected raceways, and focus the first overloaded row. **Show all raceways** clears the focused view. A ductbank handoff loads every conduit and every assigned cable in the parent ductbank, then highlights the selected conduit in the drawing and conduit table without filtering the other records. The Ductbank page keeps the cable key beside the cross-section, groups cable tags by conduit, and uses a labeled scale break for deep installations so the actual elevation remains explicit without creating excessive drawing whitespace.
