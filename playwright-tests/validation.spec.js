...
const e2ePage = file => `${file}${file.includes('?') ? '&' : '?'}e2e=1`;
...
test.describe('Validation & Standards — navigation', () => {
  test('validation.html is reachable from the navigation menu', async ({ page }) => {
    await page.goto(e2ePage('index.html'));
    // Open nav if needed
    const toggle = page.locator('#nav-toggle');
    if (await toggle.isVisible()) {
      await toggle.click();
    }
    const supportTrigger = page.getByRole('button', { name: 'Support' });
    if (await supportTrigger.count()) {
      await supportTrigger.click();
    }
    const navLink = page.locator('a[href="validation.html"]').first();
...
test.describe('Validation & Standards — study basis panels', () => {
  test('arcFlash.html has a Calculation Basis panel', async ({ page }) => {
    await page.goto(e2ePage('arcFlash.html'));
