...
|---|---|---|
| **Arc-flash label overlay on diagram** | ETAP 2024/2025 (arc flash label printing from one-line), EasyPower 2025 (arc flash annotation blocks) | These tools generate warning-label data blocks on the one-line. CableTrayRoute stores incident energy, arc-flash boundary, working distance, clearing time, and the incident-energy PPE-selection basis. Final field labels still require project safety review and approval. |
...
2. **One-Line Diagram Overlay** — `renderArcFlashLabelOverlays(svg)` in `oneline.js` renders compact WARNING badges with incident energy and the incident-energy PPE-selection basis. Signal-word severity is not inferred from incident energy alone.

Generated drafts default to an orange **WARNING** banner. **DANGER** is used only when explicitly supplied from a project hazard assessment; it is not inferred from a 40 cal/cm² threshold. Label template: `reports/labels.mjs`; tests: `tests/arcFlashLabels.test.mjs`; documentation: `docs/arc-flash-labels.md`.
...
| Missing Feature | Competitor(s) | Description |
|---|---|---|
| **Cal/cm² incident energy curve overlaid on TCC log-log chart** | ETAP 2024/2025, EasyPower 2025 | The curve shows, for a given fault current, the maximum clearing time for a user-selected incident-energy threshold. It is an incident-energy coordination aid and does not convert the selected threshold into a PPE category. |

**Status:** ✅ **Implemented 2026-04-06; labeling corrected 2026-07-15.** `incidentEnergyLimitCurve()` produces clearing-time curves for user-selected 8, 25, or 40 cal/cm² thresholds. The controls describe these as incident-energy thresholds, not PPE categories.
...
**Status:** ✅ **Implemented 2026-04-18; corrected 2026-07-15.** `analysis/dcShortCircuit.mjs` uses `I_bf = V_oc / (R_battery + 2R_cable(one-way) + R_bus)`, the iterative Stokes–Oppenlander arc-voltage model, the Ammerman open-air energy-density equation `E_arc/(4πd²)`, and the Ammerman/Wilkins LV-switchgear enclosure equation `kE_arc/(a²+d²)`. Unit conversion to cal/cm² is explicit. Results report the incident-energy PPE-selection method and a conservatively rounded-up minimum arc rating; they do not convert calculated energy into a task-based PPE category.
...
**Status:** ✅ **Implemented.** `analysis/iec60909.mjs` — voltage factor c per Table 1 (`cFactor()`), peak factor κ = 1.02 + 0.98 × e^(−3/(X/R)) (`kappaIEC()`), thermal m-factor for DC component heating (`thermalMFactor()`), transformer impedance correction K_T = 0.95 × c_max / (1 + 0.6 × xT) (`transformerCorrectionKT()`), and all four initial fault currents (I″k3 three-phase, I″k2 line-to-line, I″k1 line-to-ground, I″k2E double-line-to-ground), peak current ip, breaking current Ib (far-from-generator: Ib = I″k3), and thermal equivalent current Ith = I″k3 × √(m + n) (`computeIEC60909Bus()`). Batch runner `runIEC60909Batch()` for multi-bus studies. Integrated into `runShortCircuit()` dispatcher (`analysis/shortCircuit.mjs`) when `method === 'IEC'`. UI: `iec60909.html` — c-factor mode selector, LV tolerance, fault duration, frequency; results table with all IEC output columns; PDF and CSV export; studyBasis and studyApproval panels. Navigation: added to Studies → Protection group in `src/components/navigation.js`. Build: `iec60909: 'src/iec60909.js'` in `rollup.config.cjs`. Sitemap: `iec60909.html` added. Tests: `tests/iec60909.test.mjs` — 34 assertions covering cFactor, kappaIEC, thermalMFactor, transformerCorrectionKT, computeIEC60909Bus (all output fields, c-mode, LV tolerance, L-L ratio, Ib=Ik3, Ith≥Ik3), and runIEC60909Batch. Docs: `docs/iec60909.md` (referenced from validation benchmarks).
...
**Status:** ✅ **Implemented 2026-05-04.** `analysis/voltageFlicker.mjs` — IEC 61000-4-15 Annex A simplified rectangular voltage-change method: `calcVoltageDip()` (Thevenin ΔV% formula per IEC 61000-3-3 §4), `pstFromTable()` (bilinear log-log interpolation on iso-Pst contour matrix), `pltFromPst()` (cube-root-of-mean-cubes formula per IEC 61000-4-15 §4.7), `classifyFlickerRisk()` (Pst ≤ 0.8 pass / ≤ 1.0 marginal / > 1.0 fail), `runVoltageFlickerStudy()` (end-to-end study runner with multi-step aggregation and Plt estimation). `voltageflicker.html` + `voltageflicker.js` — full study UI: system data (S_sc kVA, X/R, nominal kV), dynamic multi-row load step schedule (label, kW, events/hr, load type), optional measured Pst series for Plt, results panel (worst Pst and Plt summary cards, per-step table, SVG bar chart with pass/marginal/fail colour coding, warnings), `initStudyBasisPanel` wired, results persisted to `dataStore`. Navigation: `Voltage Flicker` in `Studies → Power Quality` in `src/components/navigation.js`. Dashboard: `voltageFlicker` key added to `STUDY_DEFINITIONS` in `src/workflowDashboard.js` and `src/scenarioComparison.js`. `site.js` page-type regex updated to classify `voltageflicker` as `'analysis'`. Tests: `tests/voltageFlicker.test.mjs` — 10 test suites covering constants, `calcVoltageDip`, `pstFromTable` (monotonicity, bilinear interpolation, clamping), `pltFromPst` (arithmetic, edge cases), `classifyFlickerRisk` (boundary values), `runVoltageFlickerStudy` (result structure, worst-case aggregation, `pstLimitPct`, measured series, conservative estimate, validation errors).
...
| **Voltage Flicker (IEC 61000-4-15 Pst/Plt)** | **✓** | Yes | Yes | — | — | — | — | — | — | — |
...
1. ~~**IEC 60909 Short-Circuit Method** (Gap #68)~~ — ✅ **Implemented.** `analysis/iec60909.mjs`, `iec60909.html`, `src/iec60909.js`, `tests/iec60909.test.mjs`. Integrated into `runShortCircuit()` dispatcher. Navigation and sitemap added.
...
**Status:** ✅ **Implemented 2026-04-30.** `data/heatTraceProducts.json` — product family catalog (8 entries: sr-3-240, sr-5-240, sr-10-240, cw-8-240, cw-15-240, sr-8-haz, mi-20-240, mi-30-haz) covering self-regulating, constant-wattage, mineral-insulated, and hazardous-area variants with voltage list, W/ft, max circuit length per voltage, exposure rating, startup-current multiplier, and hazardous-area classification. `analysis/heatTraceSizing.mjs` — 5 new exported pure functions: `selectHeatTraceProduct(circuit, catalog)` (ranks catalog by W/ft match, voltage, hazardous-area flag, exposure temp, circuit-length check), `calcAccessoryKit(circuit, product)` (counts power connection kits, end seals, splice/tee kits, labels, controller I/O points, valve kits, flange-pair kits from circuit geometry), `buildLineList(circuits, catalog)` (returns line-list rows: pipe tag, service, area, maintain/ambient temps, effective length, selected product, W/ft, circuit kW, amps, max-length warning, controller tag, panel source), `buildHeatTraceBOM(lineListRows)` (aggregates 8 standard BOM items across all circuits), `buildControllerSchedule(lineListRows)` (groups by controller tag, returns schedule rows with circuit count, total amps, monitoring type). `heattracesizing.html` — 4 new tabs (Line List, BOM, Controller Schedule, Installation Notes) with auto-select button, product catalog filter sidebar (type / voltage / hazardous checkboxes), per-tab CSV export, engineer review panel on Line List tab, and "Export Package (.xlsx)" button for multi-sheet XLSX. `heattracesizing.js` — wiring for all new tabs, `attachPackageHandlers()`, `exportPackageXlsx()` (4 sheets: Line List, BOM, Controller Schedule, Assumptions), and `initStudyApprovalPanel` hooked to the line-list review panel. `tests/heatTraceSizing.test.mjs` — 6 new describe suites (34+ assertions) covering catalog schema, `selectHeatTraceProduct`, `calcAccessoryKit`, `buildLineList`, `buildHeatTraceBOM`, and `buildControllerSchedule`.
...
**Status:** ✅ **Implemented 2026-05-06.** `analysis/hazAreaClassification.mjs` — pure calculation module: `classifyArea()` (validates NEC Class/Division or IEC Zone descriptors, populates `_iecZoneEquiv` cross-reference), `checkEquipmentCompatibility()` (three-check engine: Ex protection type vs. zone, equipment group hierarchy IIC > IIB > IIA / IIIC > IIIB > IIIA, T-rating comparison), `checkAllEquipment()` (batch checker returning pass/fail/warn counts), `classificationReport()` (area summary table with status), `runHazAreaStudy()` (unified entry point with input validation). Constants: `NEC_CLASSES`, `NEC_DIVISIONS`, `NEC_GAS_GROUPS`, `NEC_DUST_GROUPS`, `IEC_GAS_ZONES`, `IEC_DUST_ZONES`, `IEC_EQUIPMENT_GROUPS`, `EX_PROTECTION_TYPES` (16 protection types with permitted gas/dust zones), `T_RATINGS` (T1–T6 with maxTempC), `NEC_DIV_TO_IEC_ZONE` (Table 505.7(A) equivalence mapping). Study page `hazareaclassification.html` / `hazareaclassification.js` — two-tab layout: **Classified Areas** (dynamic area rows with NEC/IEC standard toggle, group and T-rating selectors, run study button, area summary table, CSV export) and **Equipment Checker** (equipment rows with Ex protection/group/T-rating/cert-number fields, batch compatibility results table). One-line diagram integration (`oneline.js`): `showHazAreaOverlay` state + `renderHazAreaOverlay()` function draws translucent zone rectangles around components sharing a `hazAreaId`, colored by zone severity (Zone 0/Div 1 = red, Zone 1 = orange, Zone 2 = yellow), with fail badges on non-compliant devices. Design Rule Checker (`analysis/designRuleChecker.mjs`): `DRC-08` rule added — `checkHazAreaCompatibility()` converts equipment failures and warnings into DRC findings; wired into `runDRC()` via `options.hazAreaCheckResult`. Report Package (`analysis/reportPackage.mjs`): `hazAreaClass` section added to `SECTION_REGISTRY`; new `hazardousArea` preset. Navigation: `Hazardous Area Classification` added to Studies → Protection. Rollup: `hazareaclassification: 'src/hazareaclassification.js'`. Tests: `tests/hazAreaClassification.test.mjs` (57 assertions covering constants, classifyArea NEC/IEC validation, protection type/zone checks, group hierarchy, T-rating comparison, batch checking, report structure, and integration study). Docs: `docs/haz-area-classification.md`.
...
**Status:** ✅ **Implemented 2026-05-05.** `analysis/busDuctSizing.mjs` — pure calculation module: `necOrientationDerating()` (NEC 368.12: horizontal/vertical = 1.0, edge-on = 0.80), `ambientTempCorrectionFactor()` (NEC 310.15 analogy for 75°C-rated busway, reference 40°C), `stackingDerating()` (1 run = 1.00; 2 = 0.80; 3 = 0.70; ≥ 4 = 0.65), `necAmpacityDerating()` (combined three-factor chain), `voltageDropBusDuct()` (three-phase and single-phase impedance method, NEC 215.2(A) 3% threshold), `busStressForcePerFt()` (IEEE 605: F/L = 0.54 × I_kA² / d_in [lbf/ft]), `maxSupportSpan()` (simply-supported beam: L = √(8 × S_y × Z / 12 / F)), `selectStandardBusway()` (800–5000 A table), `runBusDuctStudy()` (unified study runner with auto-upsize, warnings, and structured result). `BUSWAY_LIBRARY` with indicative Al/Cu resistance, reactance, and weight for all 10 standard ratings. Study page `busdust.html` / `busdust.js` — material/phase/voltage/current/length/orientation/ambient/stacking/fault/spacing/span inputs; ampacity card with utilization badge; voltage drop card with NEC pass/fail; fault-stress card with span pass/fail; warning panel; CSV export; engineer review via `initStudyApprovalPanel('busDuctSizing')`. Navigation: `Bus Duct Sizing` added to Studies → Equipment Sizing in `src/components/navigation.js`. Rollup: `busdust: 'src/busDuct.js'`. Dashboard: `busDuctSizing` key added to `STUDY_DEFINITIONS` in `src/workflowDashboard.js`. Tests: `tests/busDuctSizing.test.mjs` (50 assertions covering all helper functions, validation paths, and integration). Docs: `docs/bus-duct-sizing.md`.
...
**Status:** ✅ **Implemented 2026-05-06.** `analysis/auditLog.mjs` — `buildEntry()`, `appendAuditEntry()`, `queryAuditLog()`, `verifyEntry()`, `computeEntryHash()`. Append-only NDJSON log at `server_data/auditLog.ndjson`; SHA-256 tamper-detection hash on every entry. `server.mjs` — `ROLE_LEVELS` and `requireRole(minRole)` RBAC middleware; `requireEngineer` applied to all project/library write routes; `/api/v1/admin/users` (GET), `/api/v1/admin/users/:username/role` (PATCH), `/api/v1/admin/audit-log` (GET) admin API routes; login response extended with `role`; audit entries written on LOGIN, project CREATE/UPDATE, and ROLE_CHANGE events. OIDC: `/auth/oidc/login` (PKCE S256 initiation), `/auth/oidc/callback` (code exchange + JIT provisioning), `/auth/oidc/logout`; configurable via `OIDC_ISSUER`, `OIDC_CLIENT_ID`, `OIDC_CLIENT_SECRET`, `OIDC_REDIRECT_URI`; JIT-provisioned users default to `reviewer` role. `admin.html` + `src/admin.js` — user management table with inline role-change dropdowns, filterable audit log viewer, CSV export. `oidc-relay.html` — browser-side session token relay page. `login.html` + `auth.js` — "Sign in with SSO" button (auto-hidden when OIDC not configured); role stored to `localStorage` on login. `src/components/navigation.js` — Admin link in Support section visible only to admin role. Rollup: `admin` entry added. Navigation: `admin.html` added to Support section (admin-only). Tests: `tests/auditLog.test.mjs` (23 assertions), `tests/rbac.test.mjs` (27 assertions), `tests/oidc.test.mjs` (9 assertions covering full PKCE flow with stub IdP). Docs: `docs/enterprise-auth.md`.
...
| **P1** | 99 | ~~**Audit Log + SSO / Enterprise Auth**~~ | Server-side audit-log table + OIDC integration | High | ✅ Implemented 2026-05-06 |
