...
**Current status: 68 of 71 total identified gaps implemented. 1 deferred (BIM/CAD plugin). Live pricing gap extended with custom CSV pricing book. 3 open (Gaps #64, #65, #70) — all represent advanced transmission-level studies. Gap #67 (Differential Protection) implemented 2026-04-18.**
...
**Status:** ✅ **Implemented 2026-04-18.** `analysis/differentialProtection.mjs` — five pure calculation functions: (1) `percentDifferentialCharacteristic()` — dual-slope operating/restraint threshold with flat, Slope 1, and Slope 2 regions; (2) `ctRatioMismatch()` — tap factor, mismatch %, and IEEE C37.91 10% limit check; (3) `harmonicRestraintCheck()` — 2nd-harmonic inrush blocking and 5th-harmonic overexcitation restraint for 87T; (4) `protectionZoneCheck()` — OPERATE / RESTRAIN / BLOCKED decision (harmonic restraint has priority for 87T); (5) `runDifferentialProtectionStudy()` — unified entry point persisted to `studies.differentialProtection` via `dataStore.mjs`. Three device entries added to `data/protectiveDevices.json`: `diff_bus_87b` (IEEE C37.97), `diff_xfmr_87t` with `harmonicRestraint: true` (IEEE C37.91), `diff_gen_87g` with lower slopes 10/25% (IEEE C37.101). TCC integration: `buildDifferentialLibraryEntries()` added to `analysis/tcc.js`; differential devices appear in a new "Differential Relays (87B/T/G)" library group; orange dashed curves (`5,3`) on the TCC chart; "Differential Protection (87B/T/G)" view toggle added. UI: `differentialprotection.html` with characteristic curve canvas visualization. Navigation: Protection group entry added to `src/components/navigation.js`. Bundle: `rollup.config.cjs` entry added. Tests: `tests/tcc/differentialProtection.test.mjs` (45 assertions). Docs: `docs/differential-protection.md`.
