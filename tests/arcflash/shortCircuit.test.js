...
(async () => {
  ...
  describe('short circuit engine', () => {
    it('handles multiple voltages with source contributions', () => {
      ...
      const a = res.bus13kV;
      assert(Math.abs(a.threePhaseKA - 23.9) < 0.1);
      assert(Math.abs(a.lineToGroundKA - 20.49) < 0.1);
      assert(Math.abs(a.lineToLineKA - 20.7) < 0.1);
      assert(Math.abs(a.doubleLineGroundKA - 44.82) < 0.1);
      const b = res.bus480V;
      assert(Math.abs(b.threePhaseKA - 19.4) < 0.1);
      assert(Math.abs(b.lineToGroundKA - 19.4) < 0.1);
      assert(Math.abs(b.lineToLineKA - 16.8) < 0.1);
      assert(Math.abs(b.doubleLineGroundKA - 38.8) < 0.1);
    });
  });
})();
