...
describe('runValidation - relay_87 required attributes', () => {
  ...
  it('flags a relay_relay_87 subtype generated by palette flow when required fields are missing', () => {
    const components = [
      {
        id: 'relay-87-3',
        type: 'relay',
        subtype: 'relay_relay_87',
        props: {
          tag: '',
          description: '',
          manufacturer: '',
    ...
    const issues = runValidation(components, {});
    const relayIssue = issues.find(issue => issue.component === 'relay-87-3');
    assert.ok(relayIssue);
    assert.ok(relayIssue.message.includes('protected_zone_type'));
  });
